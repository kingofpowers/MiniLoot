			...
			parse = function(self, tokens, matches)
				...
				if tokens[2] == token.STRING then
					...
					if tokens[3] == token.NUMBER then
						...
						for k,v in pairs(paragonReputation) do
							local repnow,max,_,can_get = C_Reputation.GetFactionParagonInfo(v["id"])
							local getrep = 0
							if can_get then
								getrep = max
							end
							repnow = repnow % max + getrep
							if repnow > v["now"] then
								data.value = repnow - v["now"]
								v["now"] = repnow
							end
						end
					end
...
do
	function ns.util:toNumber(i, prefix, separator, noColor)
		...
		if separator == true and (i <= -10000 or i >= 10000) then
			n = ns.util:ShortValue(i) --FormatLargeNumber(i) -- Util.lua:215
		end
		...
		if icon then
			...
			if appendItemLevel == true and ns.config.bool:read("ITEM_SHOW_ITEM_LEVEL") then
				...
				if itemLevel and itemLevel > 1 and (equipSlot or not ns.config.bool:read("ITEM_SHOW_ITEM_LEVEL_ONLY_EQUIPMENT")) then
					appendText = appendText .. "(" .. itemLevel .. ")"
				end
			end
	...
	function ns.util:ShortValue(value)
		local fmt
	...
	--	return "|cff" .. (value < 0 and "FF0000" or "00FF00") .. (prefix and (value < 0 and "-" or "+") or "") .. fmt:format(value) .. "|r"
	end
end
