	...
	do
		...
		function interval:Tick()
			...
			for i = 1, #summaryOrder do
				...
				if key == "experience" and playerXP>0 then
					local xpnow=UnitXP("player")
					if xpnow ~= playerXP then
						if xpnow<playerXP then
							xpnow=xpnow+playerMaxXP
						end
						local addxp=xpnow-playerXP
						entries={{addxp}}
					...
					if key == "reputation" then
						...
						for j = 1, #sorted do
							...
							if value ~= 0 then
								...
								if paragonReputation[faction] then
									tempLine = tempLine.."("..paragonReputation[faction]["now"].."/"..paragonReputation[faction]["max"]..")"
								end
...
do
	function ns.core:on()
		...
		-- register filters
		ns.events.filters:on("PLAYER_XP_UPDATE", ns.core.PARSE_CHAT)
	...
	function ns.core:off()
		...
		-- unregister filters
		ns.events.filters:off("PLAYER_XP_UPDATE", ns.core.PARSE_CHAT)
...
local function Init()
	...
	for i=1, NUM_FACTIONS_DISPLAYED do
		...
		--print("debug:LOAD function"..name.."("..factionID..")")
		if ( factionID and C_Reputation.IsFactionParagon(factionID) ) then
			local now,max,_,can_get = C_Reputation.GetFactionParagonInfo(factionID)
			local getrep = 0
			if can_get then
				getrep = max
			end
			paragonReputation[name] = { ["id"]=factionID , ["now"]=now % max + getrep , ["max"]=max }
		end
	end
end
...
ns.events:on("PLAYER_LOGIN", Init)
